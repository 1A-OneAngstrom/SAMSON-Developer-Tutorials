...
public:
    ...
    /// \brief Constructs the physical interval vector [ [\p x[0], \p x[1]] [\p y[0], \p y[1]] [\p z[0], \p z[1]] ]

    SBDTypePhysicalIAVector3Wrapper(const std::vector<Units>& x, const std::vector<Units>& y, const std::vector<Units>& z) {

        if (x.size() != 2 || y.size() != 2 || z.size() != 2) throw std::runtime_error("The size of each input vector should be 2");

        i = {   SBDTypePhysicalIntervalWrapper<Units>(x), 
                SBDTypePhysicalIntervalWrapper<Units>(y[0].getUnitScaledTo(x[0]), y[1]),
                SBDTypePhysicalIntervalWrapper<Units>(z[0].getUnitScaledTo(x[0]), z[1])};
	...
	SBDTypePhysicalIAVector3Wrapper(const std::vector<std::vector<Units>>& v) {
        ...
        for (auto u: v) {
			...
			i.push_back(SBDTypePhysicalIntervalWrapper<Units> ( u[0].getUnitScaledTo(v[0][0]), u[1] )); // scale units to the first one - checks that all units are of the same type
	...
	SBDTypePhysicalIAVector3Wrapper(const SBDTypePhysicalVector3Wrapper<Units>& u) {
    ...
    /// \brief Constructs the dimensionless physical interval vector [ [\p x[0], \p x[1]] [\p y[0], \p y[1]] [\p z[0], \p z[1]] ]

    template<typename t = Units, typename std::enable_if<std::is_same<t, SBDQuantityWrapperSI>::value, int>::type = 0>
    SBDTypePhysicalIAVector3Wrapper(const std::vector<double>& x, const std::vector<double>& y, const std::vector<double>& z) {

        if (x.size() != 2 || y.size() != 2 || z.size() != 2) throw std::runtime_error("The size of each input vector should be 2");

        i = {   SBDTypePhysicalIntervalWrapper<Units>(Units(x[0]), Units(x[1])),
                SBDTypePhysicalIntervalWrapper<Units>(Units(y[0]), Units(y[1])),
                SBDTypePhysicalIntervalWrapper<Units>(Units(z[0]), Units(z[1])) };
    ...
    /// \brief Returns the arbitrary SBPhysicalIAVector3
...
/*
	...
	bool												overlaps(const SBDTypePhysicalIAVector3Wrapper<Units>& u, const SBDTypeSymmetryTransformWrapper<Units>& t) const {
...
*/
    /// \brief Returns the overlap type (as a string) between this physical interval vector and physical interval vector \p u

	std::string											overlapsAdvanced(const SBDTypePhysicalIAVector3Wrapper<Units>& u, const Units& cutoff, const Units& cutoff2) const {
...
/*
    /// \brief Returns the overlap type (as a string) between this physical interval vector and physical interval vector \p u transformed by \p t

	std::string 												overlapsAdvanced(const SBDTypePhysicalIAVector3Wrapper<Units>& u, const Units& cutoff, const Units& cutoff2, const SBDTypeSymmetryTransformWrapper<Units>& t) const {
...
*/
    ...
    void												expand(const double r) {

        if (!isDimensionless()) throw std::runtime_error("Error, this function may only be used for dimensionless physical interval vectors");
    ...
    /// \brief Shrinks all components of this physical interval vector by offset \p r
...
/// \brief Returns the product of physical interval \p d with physical interval vector \u

template<typename Units>
SBDTypePhysicalIAVector3Wrapper<Units>				operator*(const SBDTypePhysicalIntervalWrapper<Units>& d, const SBDTypePhysicalIAVector3Wrapper<Units>& u) {
