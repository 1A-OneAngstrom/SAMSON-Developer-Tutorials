...
SBVector<T>* from1DPyArray (std::vector<SBDQuantityWrapperSI>& pyArr) {
	...
	SBVector<T>* arr = new SBVector<T>(static_cast<unsigned int>(size1stDimension));
...
SBVector<SBPhysicalVector3<T>>* from1DPyArray (std::vector<SBDTypePhysicalVector3WrapperSI>& pyArr) {
	...
	SBVector<SBPhysicalVector3<T>>* arr = new SBVector<SBPhysicalVector3<T>>(static_cast<unsigned int>(size1stDimension));
...
SBVector<SBVector<T>*>* from2DPyArray (std::vector< std::vector<SBDQuantityWrapperSI> >& pyArr) {
	...
	SBVector<SBVector<T>*>* arr = new SBVector<SBVector<T>*>(static_cast<unsigned int>(size1stDimension));

	for (std::size_t t = 0; t < size1stDimension; t++) {

		size2ndDimension = pyArr[t].size();

		SBVector<T>* currentArrT = new SBVector<T>(static_cast<unsigned int>(size2ndDimension));
...
SBVector<SBVector<SBPhysicalVector3<T>>*>* from2DPyArray (std::vector< std::vector<SBDTypePhysicalVector3WrapperSI> >& pyArr) {
	...
	SBVector<SBVector<SBPhysicalVector3<T>>*>* arr = new SBVector<SBVector<SBPhysicalVector3<T>>*>(static_cast<unsigned int>(size1stDimension));

	for (auto t = 0; t < size1stDimension; t++) {

		auto size2ndDimension = pyArr[t].size();

		SBVector<SBPhysicalVector3<T>>* currentArrT = new SBVector<SBPhysicalVector3<T>>(static_cast<unsigned int>(size2ndDimension));
