    ...
    */
	...
	std::string						toStdString_SI(const bool fullName = false) const {
		...
		if (isDimensionless()) {
            ...
            ret = toString(value) + " (dimensionless)";
            return ret;
...
#else
        // output in a scientific notation
...
/// Quantity is of SBQuantity type
/// Units is of SBDQuantityWrapper type
///
...
/// \brief Converts in the given Units from the given quantity \p a to a quantity that corresponds to SBQuantity::Quantity.
/// The conversion is done via SBQuantity::Quantity.
/// This can be used to convert from picometer to meter in Python quantities.
/// 
/// Quantity is of SBQuantity type
/// Units is of SBDQuantityWrapper type
///
template<typename Quantity, typename Units>
Units			                    convertQuantity(const Units& a) {

    return Units( a.template toSBQuantity<Quantity>() );

}

/// \brief Converts UnitsA to UnitsB: UnitsA -> SBQuantityA -> SBQuantityB -> UnitsB
/// The conversion is done via SBQuantity.
/// This can be used to convert from one unit system to another, e.g. from Dalton to SI when converting Da to kg.
/// 
/// QuantityA, QuantityB are of SBQuantity type
/// UnitSystemA, UnitSystemB are types of units used in SBDQuantityWrapper (e.g., SBUnitSystemSI, SBUnitSystemAU, etc)
///
...
namespace SBDQuantityWrapper_math {
    ...
    SBDQuantityWrapper<System>			fabs(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::fabs(q.getValue()), q.getScale(), q.getExponent());
    ...
    SBDQuantityWrapper<System>			floor(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::floor(q.getValue()), q.getScale(), q.getExponent());
    ...
    SBDQuantityWrapper<System>			ceil(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::ceil(q.getValue()), q.getScale(), q.getExponent());
    ...
    SBDQuantityWrapper<System>			round(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::round(q.getValue()), q.getScale(), q.getExponent());
    ...
    SBDQuantityWrapper<System>			trunc(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::trunc(q.getValue()), q.getScale(), q.getExponent());
    ...
    SBDQuantityWrapper<System>			pow(const SBDQuantityWrapper<System>& q, int p) {
        ...
        return SBDQuantityWrapper<System>(std::pow(q.getValue(), static_cast<double>(p)), q.getScale(), e);
    ...
    SBDQuantityWrapper<System>			sqrt(const SBDQuantityWrapper<System>& q) {
        ...
        return SBDQuantityWrapper<System>(std::sqrt(q.getValue()), q.getScale(), e);
    ...
    SBDQuantityWrapper<System>			root(const SBDQuantityWrapper<System>& q, int p) {
        ...
        return SBDQuantityWrapper<System>(std::pow(q.getValue(), 1.0 / static_cast<double>(p)), q.getScale(), e);
