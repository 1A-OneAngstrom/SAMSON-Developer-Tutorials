...
public:
	...
	/// \brief Constructs a dimensionless physical interval with all components set to \p x

	template<typename t = Units, typename std::enable_if<std::is_same<t, SBDQuantityWrapperSI>::value, int>::type = 0>
	explicit SBDTypePhysicalIntervalWrapper(double x) : i{ Units(x), Units(x) } {}

	/// \brief Constructs a dimensionless physical interval [\p x, \p y]

	template<typename t = Units, typename std::enable_if<std::is_same<t, SBDQuantityWrapperSI>::value, int>::type = 0>
	explicit SBDTypePhysicalIntervalWrapper(double x, double y) : i{ Units(x), Units(y) } {}

	/// \brief Constructs the dimensionless physical interval [\p v[0], \p v[1]]

	template<typename t = Units, typename std::enable_if<std::is_same<t, SBDQuantityWrapperSI>::value, int>::type = 0>
	SBDTypePhysicalIntervalWrapper(const std::vector<double>& u) {

		if (u.size() != 2) throw std::runtime_error("The size of the input vector should be 2");

		i = { Units(u[0]), Units(u[1]) };
    ...
    /// \brief Returns the arbitrary SBPhysicalInterval
	...
	/// \brief Returns the result of the product of this physical interval by double \p d
    ...
    /// \brief Returns the result of the division of this physical interval by double \p d
	...
	/// \brief Determines whether the intersection between this physical interval and the physical interval \p u is empty,

	bool														isEmpty(const SBDTypePhysicalIntervalWrapper<Units>& u) const {

		return isEmpty(u.i[0], u.i[1]);

	}

    /// \brief Determines whether the intersection between this physical interval and the physical interval [\p l, \p r] is empty,

	bool														isEmpty(const Units& l, const Units& r) const {
	...
	bool														contains(const Units& v) const {
	...
	/// \brief Returns true when this dimensionless physical interval contains \p v

	bool														contains(const double v) const {

		if (!isDimensionless()) throw std::runtime_error("Error, this function may only be used for dimensionless physical intervals");

		if (v < i[0].getValue()) return false;
		if (v > i[1].getValue()) return false;
		return true;

	}

	/// \brief Returns true when this physical interval contains the physical interval \p j

	bool														contains(const SBDTypePhysicalIntervalWrapper<Units>& j) const {

		if (!contains(j.i[0])) return false;
		if (!contains(j.i[1])) return false;
		return true;
	...
	/// \brief Enlarges this dimensionless physical interval to contain \p v

	void														bound(const double v) {

		if (!isDimensionless()) throw std::runtime_error("Error, this function may only be used for dimensionless physical intervals");

		if (v < i[0].getValue()) i[0] = Units(v);
		if (v > i[1].getValue()) i[1] = Units(v);
	...
	/// \brief Expands this dimensionless physical interval by offset \p r
	///
	/// \param r The offset size
	///
	/// This function expands this dimensionless physical interval by offset \p r: [\p a, \p b] becomes [\p a-r, \p b+r].
	...
	/// \brief Shrinks this dimensionless physical interval by offset \p r
	///
	/// \param r The offset size
	///
	/// This function shrinks this dimensionless physical interval by offset \p r: [\p a, \p b] becomes [\p a+r, \p b-r]. When the offset \p r is larger than the half-size of the physical interval,
	/// the interval becomes [\p m, \p m], where \p m is the center of the interval.
