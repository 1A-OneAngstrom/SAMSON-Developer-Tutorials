...
public:
	...
	SBPhysicalVector6Wrapper<Units>									getRow(const unsigned int i) const {

		if (i >= 6) throw std::runtime_error("Index out of range");

        if (i == 0) return getRow1();
        if (i == 1) return getRow2();
        if (i == 2) return getRow3();
        if (i == 3) return getRow4();
        if (i == 4) return getRow5();
		return getRow6();
	...
	SBPhysicalVector6Wrapper<Units>									getColumn(const unsigned int i) const {
    ...
    /// \brief Returns the arbitrary SBPhysicalMatrix66
	...
	/// \brief Returns the product of this spatial matrix with double \p d
	...
	/// \brief Multiplies this spatial matrix with double \p d
    ...
    /// \brief Returns the division of this spatial matrix by double \p d
    ...
    /// \brief Divides this spatial matrix by double \p d
    ...
    /// \brief Returns true if this spatial matrix is equal to spatial matrix \p mat (component-wise)

    bool														operator==(const SBDTypePhysicalMatrix66Wrapper<Units>& mat) const {

        if (m00 != mat.m00) return false;
        if (m01 != mat.m01) return false;
        if (m10 != mat.m10) return false;
        if (m11 != mat.m11) return false;
        return true;

    }

    /// \brief Returns true if this spatial matrix is different from spatial matrix \p mat (component-wise)

    bool														operator!=(const SBDTypePhysicalMatrix66Wrapper<Units>& mat) const {

        if (m00 != mat.m00) return true;
        if (m01 != mat.m01) return true;
        if (m10 != mat.m10) return true;
        if (m11 != mat.m11) return true;
        return false;
