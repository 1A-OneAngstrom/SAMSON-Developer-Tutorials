...
public:
	...
	SBDTypePhysicalMatrix33Wrapper(const SBDTypePhysicalVector3Wrapper<Units>& c0, const SBDTypePhysicalVector3Wrapper<Units>& c1, const SBDTypePhysicalVector3Wrapper<Units>& c2) {
        ...
        m = {{ c0.v[0], c1.v[0].getUnitScaledTo(c0.v[0]), c2.v[0].getUnitScaledTo(c0.v[0]) },
             { c0.v[1], c1.v[1].getUnitScaledTo(c0.v[0]), c2.v[1].getUnitScaledTo(c0.v[0]) },
             { c0.v[2], c1.v[2].getUnitScaledTo(c0.v[0]), c2.v[2].getUnitScaledTo(c0.v[0]) }};
	...
	SBDTypePhysicalMatrix33Wrapper(double m00, double m01, double m02, double m10, double m11, double m12, double m20, double m21, double m22) {
        ...
        m = {   { Units(m00), Units(m01), Units(m02) }, 
                { Units(m10), Units(m11), Units(m12) }, 
                { Units(m20), Units(m21), Units(m22) } };
    ...
    /// \brief Returns the column \p i of this physical matrix

    SBPhysicalVector3Wrapper<Units>								getColumn(const unsigned int i) const {

        if (i >= 3) throw std::runtime_error("Index out of range");

        return SBPhysicalVector3Wrapper<Units>(m[0][i], m[1][i], m[2][i]);

    }

    /// \brief Returns the row \p i of this physical matrix

	SBPhysicalVector3Wrapper<Units>								getRow(const unsigned int i) const {

		if (i >= 3) throw std::runtime_error("Index out of range");

		return SBPhysicalVector3Wrapper<Units>(m[i][0], m[i][1], m[i][2]);
    ...
    /// \brief Returns the arbitrary SBPhysicalMatrix33
	...
	void														diagonalize(SBDTypePhysicalVector3Wrapper<Units>& ev, SBDTypePhysicalMatrix33Wrapper<Units>& p) const {
        ...
        using SBDQuantityWrapper_math::fabs;
        using SBDQuantityWrapper_math::sqrt;
	...
	Units														norm() const {
        ...
        if (rho < SBDQuantityWrapper_math::fabs(eV.v[0])) rho = SBDQuantityWrapper_math::fabs(eV.v[0]);
        if (rho < SBDQuantityWrapper_math::fabs(eV.v[1])) rho = SBDQuantityWrapper_math::fabs(eV.v[1]);
        if (rho < SBDQuantityWrapper_math::fabs(eV.v[2])) rho = SBDQuantityWrapper_math::fabs(eV.v[2]);

        return SBDQuantityWrapper_math::sqrt(rho);
	...
	Units														cosphi() const {
        ...
        if (rho > SBDQuantityWrapper_math::fabs(eV.v[0])) rho = (eV.v[0]);
        if (rho > SBDQuantityWrapper_math::fabs(eV.v[1])) rho = (eV.v[1]);
        if (rho > SBDQuantityWrapper_math::fabs(eV.v[2])) rho = (eV.v[2]);
    ...
    void														makeEulerRotationZYZ(const Units& phi, const Units& theta, const Units& psi) {
        ...
        using SBDQuantityWrapper_math::sin;
        using SBDQuantityWrapper_math::cos;
...
/// \brief Returns the product of double \p d and matrix \p mat

template<typename Units>
SBDTypePhysicalMatrix33Wrapper<Units>		operator*(const double d, const SBDTypePhysicalMatrix33Wrapper<Units>& mat) {

	return SBDTypePhysicalMatrix33Wrapper<Units>(
        mat.m[0][0] * d, mat.m[0][1] * d, mat.m[0][2] * d,
        mat.m[1][0] * d, mat.m[1][1] * d, mat.m[1][2] * d,
        mat.m[2][0] * d, mat.m[2][1] * d, mat.m[2][2] * d
        );
