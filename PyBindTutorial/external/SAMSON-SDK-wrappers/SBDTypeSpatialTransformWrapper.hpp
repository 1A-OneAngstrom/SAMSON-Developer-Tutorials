...
/// \brief This class is used as a wrapper between SAMSON SDK SBDTypeSpatialTransform and Python.
...
public:
	...
	SBDTypeSpatialTransformWrapper() {
		...
		SBDQuantityWrapperSI qs(1.0, scaleScale, scaleExponent);
		...
		scale = SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>(qs);
	...
	SBDTypeSpatialTransformWrapper(const SBDTypePhysicalMatrix33Wrapper<SBDQuantityWrapperSI>& o,
								   const SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>& p) : SBDTypeSpatialTransformWrapper() {
	...
	/// \param orientation The orientation matrix of the spatial transform
	/// \param position The position vector of the spatial transform
	/// \param scale The scale vector of the spatial transform

	SBDTypeSpatialTransformWrapper(const SBDTypePhysicalMatrix33Wrapper<SBDQuantityWrapperSI>& o,
		const SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>& p,
		const SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>& s) : SBDTypeSpatialTransformWrapper() {

		if (o.m[0][0].getExponent() != orientationExponent || p.v[0].getExponent() != positionExponent || s.v[0].getExponent() != scaleExponent)
			throw std::runtime_error("Cannot construct a spatial transform: matrix33 should be of type dimensionless, vector3 should be of type position, scale  should be of type dimensionless");
	...
	SBDTypeSpatialTransformWrapper(const SBSpatialTransform& st) {
		...
		scale = {	SBDQuantityWrapperSI(st.scale->v[0]),
					SBDQuantityWrapperSI(st.scale->v[1]),
					SBDQuantityWrapperSI(st.scale->v[2]) };
	...
	/// \brief Returns the arbitrary SBSpatialTransform
	...
	SBSpatialTransform										toSBSpatialTransform () const {
		...
		return SBSpatialTransform(
					...
					getSBPhysicalVector3<SBQuantity::dimensionless>(scale)
					);
	...
	SBPhysicalVector3Wrapper<SBDQuantityWrapperSI>			getE5() const {

		return SBPhysicalVector3Wrapper<SBDQuantityWrapperSI>(scale.v[0], scale.v[1], scale.v[2]);
	...
	/// \brief Sets the scale vector to \p vec

	void													setE5(const SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>& vec) {

		if (vec.v[0].getExponent() != scaleExponent)
			throw std::runtime_error("Cannot set the scale of the spatial transform: types mismatch - input vector should be dimensionless");
	...
	/// \brief Returns the scale of the spatial transform

	SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>		getScale() const {
	...
	/// \brief Sets the scale of the spatial transform to \p pos

	void													setScale(const SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>& s) {

		if (s.v[0].getExponent() != scaleExponent)
			throw std::runtime_error("Cannot set the scale of the spatial transform: types mismatch - input vector should be dimensionless");
	...
	bool													operator==(const SBDTypeSpatialTransformWrapper& transform) const {
		...
		return (orientation == transform.orientation && position == transform.position && scale == scale);
...
public:
	...
	SBDTypePhysicalVector3Wrapper<SBDQuantityWrapperSI>		scale;									///< The scale of the spatial transform
...
private:
	...
	std::vector<int>										scaleExponent       = QuantityScaleExponent<SBQuantity::dimensionless>::exponent;
	std::vector<int>										scaleScale          = QuantityScaleExponent<SBQuantity::dimensionless>::scale;
...
SBDTypeSpatialTransformWrapper<Units>						operator*(const double d, const SBDTypeSpatialTransformWrapperForPython<Units>& u) {
